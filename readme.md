...
- 두 변수의 상관관계 > 상관관계
   ...
   두 변수가 상관 관계가 있는 경우
   - x축 변수 값이 증가하면 y축 변수 값이 비례해서 증가
   - x축 변수 값이 증가하면 y축 변수 값이 비례해서 감소
- 상관계수 : 상관관계를 수치로 나타낸 -1에서 1 사이의 값
   - 상관계수가 0 : 두 변수 X,Y 사이에 상관성을 찾기 어려움
   - 상관계수 (-) : X,Y가 반비례, 음의 상관관계에 있음
   - 상관계수 (+) : X,Y가 비례, 양의 상관관계에 있음
...
# 데이터 확인
st <- data.frame(state.x77) # 매트릭스를 데이터프레임을 변환
head(st)
